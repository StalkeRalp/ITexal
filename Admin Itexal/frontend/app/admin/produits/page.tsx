...
export default function PageProduitsAdmin() {
  ...
  const sauvegarderProduitHandler = (data: any) => {
    if (produitAEditer) {
      ...
      toast.succes(t("products.updatedSuccess"));
    } else {
      creerProduit({
      ...
      toast.succes(t("products.createdSuccess"));
    }
  ...
  const confirmerSuppression = () => {
    if (idASupprimer) {
      ...
      toast.succes(t("products.deletedSuccess"));
  ...
  // Filtrage
  const labelToutes = t("common.all");
  const produitsFiltres = produits.filter((p) => {
    ...
    const matchCat =
      filtreCategorie === "Toutes" || filtreCategorie === labelToutes ||
  ...
  return (
    <div className="space-y-8 animate-fadeIn max-w-[1600px] mx-auto pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        >
          <Add01Icon size={18} strokeWidth={2.5} /> {t("products.addProduct")}
        </button>
      </div>
      ...
      <div className="bg-white rounded-3xl p-4 shadow-sm border border-slate-100 flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 overflow-x-auto w-full md:w-auto pb-1 md:pb-0">
          {Array.from(new Set([labelToutes, ...categories.map((c) => c.nom)])).map((cat, idx) => (
            <button
              key={`${cat}-${idx}`}
              type="button"
              onClick={() => setFiltreCategorie(cat)}
              className={`px-3.5 py-2 rounded-xl text-xs font-bold transition-all shrink-0 cursor-pointer ${
                filtreCategorie === cat || (filtreCategorie === "Toutes" && cat === labelToutes)
        ...
        >
          ...
          {produitsFiltres.length === 0 && (
            <div className="bg-white rounded-3xl p-12 text-center border border-slate-100 space-y-3">
              ...
              <h3 className="text-base font-bold text-slate-800">{t("common.noResults")}</h3>
